...
- `craw_soyte.py` (crawler Medinet `medinet.hochiminhcity.gov.vn`, **chỉ ghi CSV local** trong `data/`)
...
## Crawl Medinet — Sở Y tế TP.HCM (`craw_soyte.py`)

Script này crawl `https://medinet.hochiminhcity.gov.vn/`, dùng cùng luồng BFS / queue / resume như `craw_thuocbietduoc.py`, nhưng:

- Trích nội dung từ phần tử DOM có **`id="MainContent"`** (site ASP.NET), không dùng thẻ `<main>`.
- **Không ghi PostgreSQL**; kết quả chỉ lưu file CSV dưới thư mục gốc output (mặc định `data/`).

### Output mặc định (`--output-root data`)

- `data/soyte_pages.csv` — mỗi URL một dòng: `url`, `title`, `content`, `source_domain`, `crawl_time_utc`, `content_root_id`
- `data/soyte_images.csv`, `data/soyte_videos.csv`, `data/soyte_files.csv`
- `data/state/events_soyte_csv_<domain>_<hash>.jsonl` — state để resume
- `data/logs/soyte_crawl.log` — log file (thêm vào root logger)
...
### Resume + retry URL đã fail

```bat
python craw_soyte.py --retry-failed-on-resume
```

Giống `craw_thuocbietduoc.py`: mặc định các URL đã `done` với lỗi sẽ không vào lại hàng đợi khi chạy tiếp; bật flag trên để crawl lại chúng.

### Các option chính (soyte)

- `--start-url <url>` (mặc định Medinet)
- `--max-pages <n>`: `0` là không giới hạn
- `--concurrency <n>`
- `--browser-concurrency <n>`
- `--recycle-every <n>`: recycle browser mỗi N lần `arun` (0 = tắt)
- `--connect-error-retry-limit <n>`
- `--page-timeout-ms <ms>`
- `--output-root <dir>`: thư mục chứa CSV, `state/`, `logs/` (mặc định `data`)
- `--no-resume`
- `--retry-failed-on-resume`
- `--allow-domain <domain>`: lặp lại được
- `--content-element-id <id>`: đổi id vùng nội dung nếu site đổi markup (mặc định `MainContent`)

### Tùy chỉnh nhanh bằng biến `DEFAULT_*`

Ở đầu `craw_soyte.py` có `DEFAULT_START_URL`, `DEFAULT_OUTPUT_ROOT`, `DEFAULT_CONTENT_ELEMENT_ID`, cùng các `DEFAULT_*` kế thừa hành vi từ `craw_thuocbietduoc.py` (concurrency, browser, recycle, …).
...
Các crawler lưu state (JSONL append-only) để resume; thư mục phụ thuộc `--output-root`:

- **`scrapling_demo/crawl4ai_benhvien_scraper.py`**, **`craw_thuocbietduoc.py`**, **`craw_boyte.py`** (mặc định): `data_craw/state/`
- **`craw_soyte.py`** (mặc định): `data/state/` (vì `DEFAULT_OUTPUT_ROOT = data`)

Tên file ví dụ:

- `events_db.jsonl` — crawl4ai bệnh viện (HTML)
- `events_pdf_db.jsonl` — PDF extract
- `events_raw_pdf_db.jsonl` — raw PDF URL
- `events_bietduoc_db_<domain>_<hash>.jsonl` — `craw_thuocbietduoc.py`
- `events_boyte_db_<domain>_<hash>.jsonl` — `craw_boyte.py`
- `events_soyte_csv_<domain>_<hash>.jsonl` — `craw_soyte.py`
...
- `events_soyte_csv_<domain>_<hash>.jsonl`: trạng thái crawl của `craw_soyte.py` (thư mục: `data/state/` nếu dùng output mặc định)
